...
namespace gspro_r10
{
  public class BluetoothConnection : IDisposable
  {
    ...
    public void LogMetrics(Metrics? metrics)
    {
      ...
      try
      {
        double ballSpeedMph = (metrics.BallMetrics?.BallSpeed ?? 0) * METERS_PER_S_TO_MILES_PER_HOUR;
        double clubSpeedMph = (metrics.ClubMetrics?.ClubHeadSpeed ?? 0) * METERS_PER_S_TO_MILES_PER_HOUR;
